...
export default function EditPostPage() {
  ...
  if (!Number.isInteger(postId) || postId <= 0) {
    return (
      <main className="mx-auto max-w-[920px] px-6 py-16">
        <p className="text-red-600">文章編號無效。</p>
      </main>
    );
  }
  if (postQuery.isLoading) {
    return (
      <main className="mx-auto max-w-[920px] px-6 py-16 text-[var(--ink-soft)]">
        正在載入文章…
      </main>
    );
  }
  if (postQuery.isError || !post) {
    return (
      <main className="mx-auto max-w-[920px] px-6 py-16">
        <h1 className="text-3xl font-bold [font-family:var(--serif)]">
          找不到文章
        </h1>
        ...
        <Link
          className="mt-6 inline-block font-semibold text-[var(--accent-ink)] underline underline-offset-4"
          to="/admin/posts"
        >
          返回文章管理
        </Link>
    ...
    <main className="mx-auto max-w-[920px] px-6 py-12">
      <header className="mb-8 flex flex-col gap-4 border-b border-[var(--rule)] pb-8 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 text-[var(--ink-soft)]">
            修改內容與發布狀態，儲存後會回到文章管理。
          </p>
        </div>
        <Link
          className="text-sm font-semibold text-[var(--ink-soft)] underline underline-offset-4 hover:text-[var(--ink)]"
          to="/admin/posts"
        >
          取消並返回
        </Link>
      ...
      >
        ...
        <button
          className="rounded-lg bg-[var(--accent-brand)] px-6 py-3 font-semibold text-white transition-colors hover:bg-[var(--accent-brand-hover)] disabled:opacity-60"
